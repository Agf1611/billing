...
async function resolveDeviceToken(input, options = {}) {
  const token = String(input || '').trim();
  if (!token) return null;

  const direct = await findDeviceByTag(token, options);
  if (direct && direct._id) return direct;

  const byPppoe = await findDeviceByPppoe(token, options);
  if (byPppoe && byPppoe._id) return byPppoe;

  const found = await findDeviceWithTagVariants(token, options);
...
async function findDeviceWithTagVariants(input, options = {}) {
  for (const c of expandTagCandidates(input)) {
    const dev = await findDeviceByTag(c, options);
...
function readNodeValue(node) {
  ...
  if (typeof node === 'object' && node._value !== undefined) return node._value;
  return node;
}
...
function parseInteger(value) {
  const raw = readNodeValue(value);
  if (raw === null || raw === undefined || raw === '') return 0;
  const parsed = Number.parseInt(String(raw).replace(/[^\d-]/g, ''), 10);
  return Number.isFinite(parsed) ? parsed : 0;
}

function detectConnectedUserStatus(entry, deviceLastInformMs = 0) {
  const rawActive = readNodeValue(entry?.Active);
  const normalizedActive = toTrimmedString(rawActive).toLowerCase();
  if (rawActive === true || rawActive === 1) return 'Online';
  if (rawActive === false || rawActive === 0) return 'Offline';
  if (['true', '1', 'yes', 'y', 'on', 'online', 'active', 'up', 'connected'].includes(normalizedActive)) {
    return 'Online';
  }
  if (['false', '0', 'no', 'n', 'off', 'offline', 'inactive', 'down', 'disconnected'].includes(normalizedActive)) {
    return 'Offline';
  }

  const leaseRemaining = parseInteger(entry?.LeaseTimeRemaining);
  if (leaseRemaining > 0) return 'Online';

  const hostStampMs = Math.max(
    parseDateMs(entry?._timestamp),
    parseDateMs(entry?.HostName?._timestamp),
    parseDateMs(entry?.IPAddress?._timestamp),
    parseDateMs(entry?.MACAddress?._timestamp),
    parseDateMs(entry?.InterfaceType?._timestamp)
  );

  if (hostStampMs > 0 && deviceLastInformMs > 0) {
    const ageMs = deviceLastInformMs - hostStampMs;
    if (ageMs <= 20 * 60 * 1000) return 'Online';
    if (ageMs > 20 * 60 * 1000) return 'Offline';
  }

  const hasIdentity = Boolean(
    toTrimmedString(readNodeValue(entry?.MACAddress)) ||
    toTrimmedString(readNodeValue(entry?.IPAddress)) ||
    toTrimmedString(readNodeValue(entry?.HostName))
  );
  if (hasIdentity && deviceLastInformMs > 0) return 'Online';
...
function mapDeviceData(device, tag) {
  ...
  let status = 'Unknown';
  let deviceLastInformMs = 0;
  if (device?._lastInform) {
    deviceLastInformMs = new Date(device._lastInform).getTime();
    const diffMs = Date.now() - deviceLastInformMs;
    status = diffMs < 15 * 60 * 1000 ? 'Online' : 'Offline';
  } else if (device?.Events?.Inform) {
    deviceLastInformMs = new Date(device.Events.Inform).getTime();
    const diffMs = Date.now() - deviceLastInformMs;
    status = diffMs < 15 * 60 * 1000 ? 'Online' : 'Offline';
  }
      ...
      for (const key in hosts) {
        if (!isNaN(key)) {
          ...
          connectedUsers.push({
            ...
            status: detectConnectedUserStatus(entry, deviceLastInformMs)
          });
        }
      }
    }
...
async function getCustomerDeviceData(tag, options = {}) {
  const base = await resolveDeviceToken(tag, options);
  if (!base || !base._id) return null;
  const device = await fetchFullDevice(base._id, options);
  return mapDeviceData(device, tag);
}

function hasMeaningfulDeviceData(data) {
  ...
  const hasCoreField = checks.some((value) => {
    const raw = String(value || '').trim();
    return raw && raw !== '-' && raw.toLowerCase() !== 'n/a' && raw.toLowerCase() !== 'unknown';
  });
  const hasConnectedUsers = Array.isArray(data.connectedUsers) && data.connectedUsers.length > 0;
  return hasCoreField || hasConnectedUsers;
}

async function requestDeviceRefresh(tag, options = {}) {
  try {
    ...
    logger.debug(`[GenieACS] requestDeviceRefresh error: ${error.message}`);
...
async function waitForCustomerDeviceData(tag, options = {}) {
  ...
  do {
    attempts += 1;
    lastData = await getCustomerDeviceData(tag, { timeoutMs });
    if (hasMeaningfulDeviceData(lastData)) {
      return {
        ok: true,
        data: lastData,
        attempts,
    ...
    if (!refreshRequested && requestRefresh) {
      ...
      await requestDeviceRefresh(tag, { timeoutMs: Math.min(timeoutMs, 5000) });
    }
