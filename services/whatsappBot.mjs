...
function normalizeDisconnectCode(rawCode) {
  ...
  return Number.isFinite(parsed) ? parsed : rawCode;
}

function getDisconnectReasonLabel(code) {
  switch (normalizeDisconnectCode(code)) {
    ...
    default:
      return code ? `code_${code}` : 'unknown';
  }
}
...
function scheduleReconnect(code, delayMs = 3000) {
  ...
  reconnectTimer = setTimeout(() => {
    reconnectTimer = null;
    whatsappStatus.reconnectAt = null;
    startWhatsAppBot({ force: true }).catch((err) => {
      logger.error(`[WA] Gagal start ulang bot: ${err.message || err}`);
    });
  }, delayMs);
}
...
export async function restartWhatsAppBot() {
  ...
  if (oldSock) {
    try {
      oldSock.end(new Error('Manual restart requested'));
    } catch (e) {
      logger.error('WhatsApp: Gagal menghentikan socket lama:', e.message);
    }
  }
  setTimeout(() => {
    startWhatsAppBot({ force: true }).catch((e) => {
      logger.error('WhatsApp: Gagal memulai ulang socket:', e.message || e);
    });
  }, 1000);
}

export async function startWhatsAppBot(options = {}) {
  const force = Boolean(options && options.force);
  if (currentStartPromise) return currentStartPromise;
  if (!force && currentSock && ['open', 'connecting', 'qr'].includes(String(whatsappStatus.connection || ''))) {
    return currentSock;
  }
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    const isCurrentSocket = currentSock === sock && startEpoch === socketEpoch;
    updateWhatsappStatus({});
    ...
    if (connection === 'close') {
      const code = normalizeDisconnectCode(lastDisconnect?.error?.output?.statusCode);
      const reasonLabel = getDisconnectReasonLabel(code);

      if (!isCurrentSocket) {
        logger.info(`[WA] Mengabaikan event close dari socket lama (kode ${code || '-'}).`);
        return;
      }
      ...
      if (qrShownSinceStart && !notifiedAdminForQr) {
        ...
        if (toList.length > 0) {
          const msg = waWrap('WHATSAPP BOT AKTIF', 'WhatsApp bot sudah aktif.');
