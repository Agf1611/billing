...
for f in \
  "$APP/routes/adminPortal.js" \
  "$APP/views/admin/settings.ejs" \
  "$APP/views/dashboard.ejs" \
  "$APP/settings.json"
do
  if [ -f "$f" ]; then cp "$f" "$f.logoupload-$TS"; fi
done
...
rm -f /home/agf16/adminPortal.js /home/agf16/settings.ejs /home/agf16/dashboard.ejs /home/agf16/settings.json /home/agf16/deploy_logo_upload.sh